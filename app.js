...
app.get('/member2023_1', (req, res) => {

    var sql = `select * from mem where generation = "2023-1" order by idx desc `
    connection.query(sql, function (err, results, fields) {
        if (err) throw err;
...
app.get('/member2023_2', (req, res) => {

    var sql = `select * from mem where generation = "2023-2" order by idx desc `
    connection.query(sql, function (err, results, fields) {
        if (err) throw err;
        res.render('member_2', { lists: results })
    })
})

app.get('/member2024_1', (req, res) => {

    var sql = `select * from mem where generation = "2024-1" order by idx desc `
    connection.query(sql, function (err, results, fields) {
        if (err) throw err;
        res.render('member_3', { lists: results })
    })
})

app.get('/member2024_2', (req, res) => {

    var sql = `select * from mem where generation = "2024-2" order by idx desc `
    connection.query(sql, function (err, results, fields) {
        if (err) throw err;
        res.render('member_4', { lists: results })
    })
})
...
app.post('/registerProc', (req, res) => {
    ...
    var sql = `insert into mem(user_id,pw,name,stu_num,phone,generation) values(?,?,?,?,?,?)`

    var values = [user_id, pw, name, stu_num, phone, generation];
